...
inline void calculate_acceleration_array(unsigned int ci, unsigned int cj, unsigned int ncside, double xp, double yp, double m, double &axF, double &ayF,  double *cell)
{
  ...
  // Intercation with own cell : 0
  // Calculate distance from cell to point positions
  double dx = (cell[3*(ci * ncside + cj)+1] - xp);
  double dy = (cell[3*(ci * ncside + cj)+2] - yp);
  // Calcuate square ditance
  double d2 = dx * dx + dy * dy;
  double fd,ax =0,ay =0;
  // Check if the distance is bigger then epslon
  ...
  {
    fd = 1 / (sqrt(d2) * d2);
    ax = G * cell[3*(ci * ncside + cj)] * fd * dx;
    ay = G * cell[3*(ci * ncside + cj)] * fd * dy;
  }
  // Intercation with right cell (I+1,J) : 1
  dx = cell[3*(cip1 * ncside + cj)+1] - xp + wr;
  dy = cell[3*(cip1 * ncside + cj)+2] - yp;
  d2 = dx * dx + dy * dy;
  if (d2 >= EPSLON2)
  {
    fd = 1 / (sqrt(d2) * d2);
    ax += G * cell[3*(cip1 * ncside + cj)] * fd * dx;
    ay += G * cell[3*(cip1 * ncside + cj)] * fd * dy;
  }
  // Interaction with botton right cell (I+1,J-1) : 2
  dx = cell[3*(cip1 * ncside + cjm1)+1] - xp + wr;
  dy = cell[3*(cip1 * ncside + cjm1)+2] - yp - wb;
  d2 = dx * dx + dy * dy;
  if (d2 >= EPSLON2)
  {
    fd = 1 / (sqrt(d2) * d2);
    ax += G * cell[3*(cip1 * ncside + cjm1)] * fd * dx;
    ay += G * cell[3*(cip1 * ncside + cjm1)] * fd * dy; //2
  }
  // Interaction with botton cell (I,J-1) : 3
  dx = cell[3*(ci * ncside + cjm1)+1] - xp;
  dy = cell[3*(ci * ncside + cjm1)+2] - yp - wb;
  d2 = dx * dx + dy * dy;
  if (d2 >= EPSLON2)
  {
    fd = 1 / (sqrt(d2) * d2);
    ax += G * cell[3*(ci * ncside + cjm1)] * fd * dx;
    ay += G * cell[3*(ci * ncside + cjm1)] * fd * dy;
  }
  // Interaction with left botton cell (I-1,J-1) : 4
  dx = (cell[3*(cim1 * ncside + cjm1)+1] - xp) - wl;
  dy = (cell[3*(cim1 * ncside + cjm1)+2] - yp) - wb;
  d2 = dx * dx + dy * dy;
  if (d2 >= EPSLON2)
  {
    fd = 1 / (sqrt(d2) * d2);
    ax += G * cell[3*(cim1 * ncside + cjm1)] * fd * dx;
    ay += G * cell[3*(cim1 * ncside + cjm1)]* fd * dy;
  }
  // Interaction with left botton cell (I-1,J) : 5
  dx = (cell[3*(cim1 * ncside + cj)+1] - xp) - wl;
  dy = (cell[3*(cim1 * ncside + cj)+2] - yp);
  d2 = dx * dx + dy * dy;
  if (d2 >= EPSLON2)
  {
    fd = 1 / (sqrt(d2) * d2);
    ax += G * cell[3*(cim1 * ncside + cj)] * fd * dx;
    ay += G * cell[3*(cim1 * ncside + cj)] * fd * dy; //5
  }
  // Interaction with left botton cell (I-1,J+1) : 6
  dx = (cell[3*(cim1 * ncside + cjp1)+1] - xp) - wl;
  dy = (cell[3*(cim1 * ncside + cjp1)+2] - yp) + wu;
  d2 = dx * dx + dy * dy;
  if (d2 >= EPSLON2)
  {
    fd = 1 / (sqrt(d2) * d2);
    ax += G * cell[3*(cim1 * ncside + cjp1)] * fd * dx;
    ay += G * cell[3*(cim1 * ncside + cjp1)] * fd * dy; //6
  }
  // Interaction with left botton cell (I,J+1) : 7
  dx = (cell[3*(ci * ncside + cjp1)+1] - xp); //7 (I,J+1)
  dy = (cell[3*(ci * ncside + cjp1)+2] - yp) + wu;
  d2 = dx * dx + dy * dy;
  if (d2 >= EPSLON2)
  {
    fd = 1 / (sqrt(d2) * d2);
    ax += G * cell[3*(ci * ncside + cjp1)] * fd * dx;
    ay += G * cell[3*(ci * ncside + cjp1)] * fd * dy; //7
  }
  // Interaction with left botton cell (I+1,J+1) : 8
  dx = (cell[3*(cip1 * ncside + cjp1)+1] - xp) + wr;
  dy = (cell[3*(cip1 * ncside + cjp1)+2] - yp) + wu;
  d2 = dx * dx + dy * dy;
  if (d2 >= EPSLON2)
  {
    fd = 1 / (sqrt(d2) * d2);
    ax += G * cell[3*(cip1 * ncside + cjp1)] * fd * dx;
    ay += G * cell[3*(cip1 * ncside + cjp1)] * fd * dy; //8
  }
                                                      //-----------------------------------------------------
  axF = ax;
  ayF = ay;
